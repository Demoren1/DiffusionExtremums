"""CLI: evaluate the oracle-target effective-map regressor (oracle baseline).

Loads a trained oracle-target regressor checkpoint, builds the exact oracle
effective maps and the learned MLP-averaged maps, evaluates the full metric
suite (normalized/raw MSE, Toeplitz-ness, kernel recovery, functional MSE for
oracle conv / target MLP / oracle regressor), saves metrics to ``--output-dir``,
and generates figures to ``--figures-dir``.
...
from src.evaluation.oracle_regressor_eval import (
    aggregate_oracle_results,
    evaluate_oracle_regressor,
    generate_oracle_figures,
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Evaluate the oracle-target effective-map regressor "
                    "(oracle-quality baseline).")
    p.add_argument("--checkpoint", type=str,
                   default="results/regressor_oracle/regressor_best.pt",
                   help="Path to the trained oracle-target regressor checkpoint.")
    ...
    p.add_argument("--output-dir", type=str,
                   default="results/regressor_oracle/evaluation",
                   help="Directory for evaluation results.")
    p.add_argument("--figures-dir", type=str,
                   default="figures/regressor_oracle",
...
def main(argv=None) -> int:
    """Run the oracle-target regressor evaluation."""
    args = parse_args(argv)
    set_seed(args.seed)
    device = torch.device(args.device)

    print("=" * 70)
    print(" Oracle-target regressor evaluation")
    print("=" * 70)
    ...
    target_mode = ckpt.get("config", {}).get("target_mode", "unknown")
    ...
    # ------------------------------------------------------------------
    # Load configs + build oracle and learned targets
    # ------------------------------------------------------------------
    print("\n[2/3] Loading configs + building oracle/learned targets...")
    ...
    learned_targets_all = eff_maps.mean(dim=1).contiguous()  # [n_configs, D_eff]
    oracle_targets_all = torch.stack([
    ...
    print(f"  learned targets: {tuple(learned_targets_all.shape)}")
    print(f"  oracle targets:  {tuple(oracle_targets_all.shape)}")
    ...
    results = evaluate_oracle_regressor(
        model=model,
        normalizer=normalizer,
        configs=configs,
        oracle_targets_all=oracle_targets_all,
        learned_targets_all=learned_targets_all,
    ...
    summary = aggregate_oracle_results(results)
    summary["target_mode"] = target_mode
    print_oracle_summary(results, summary)
    save_oracle_results(results, summary, args.output_dir)
    ...
    print("\n" + "=" * 70)
    print(" Oracle-target regressor evaluation complete.")
    print("=" * 70)
    return 0
